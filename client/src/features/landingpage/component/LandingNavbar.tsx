...
export default function LandingNavbar() {
  ...
  return (
    <>
      <header className="w-full bg-white/90 backdrop-blur-md border-b border-slate-100 sticky top-0 z-50 px-6 md:px-16" dir="rtl">
        <div className="max-w-7xl mx-auto h-20 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-10">
            <Link href="/" className="text-xl font-black text-slate-900 tracking-tight flex items-center gap-2">
              <span className="text-[#007f5f]">تعلّم</span>
            </Link>

            {/* Hidden on mobile, visible on desktop */}
            <nav className="hidden md:flex items-center gap-8 text-xs font-bold text-slate-600">
              <Link href="/" className="text-[#007f5f] hover:text-[#02664a] transition-all">الرئيسية</Link>
              <Link href="#features" className="hover:text-slate-900 transition-all">المميزات</Link>
              <Link href="#pricing" className="hover:text-slate-900 transition-all">الأسعار</Link>
            </nav>
          </div>
          ...
          <div className="flex items-center gap-4 flex-1 md:flex-initial justify-end">
            ...
            <button className="hidden sm:block bg-slate-950 hover:bg-slate-900 text-white font-bold text-xs px-6 py-2.5 rounded-xl transition-all whitespace-nowrap">
              دخول
            </button>
      ...
      <div 
        className={`fixed top-0 right-0 h-full w-[280px] bg-white shadow-2xl z-50 p-6 transform transition-transform duration-300 ease-out flex flex-col justify-between text-right ${
      ...
      >
        <div>
          ...
          <nav className="flex flex-col gap-5 mt-8 text-sm font-bold text-slate-600">
            <Link href="/" onClick={() => setIsOpen(false)} className="text-[#007f5f] py-1">الرئيسية</Link>
            <Link href="#features" onClick={() => setIsOpen(false)} className="hover:text-slate-900 py-1 transition-all">المميزات</Link>
            <Link href="#pricing" onClick={() => setIsOpen(false)} className="hover:text-slate-900 py-1 transition-all">الأسعار</Link>
          </nav>
        </div>
