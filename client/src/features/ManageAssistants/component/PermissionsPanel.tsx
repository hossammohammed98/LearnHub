...
export default function PermissionsPanel({ selectedAssistant, isSaving = false, onSubmit }: PermissionsPanelProps) {
  const [permissions, setPermissions] = useState<Permission[]>(() =>
    initialPermissions.map((permission) => ({
      ...permission,
      enabled: selectedAssistant?.permissionIds.includes(permission.id) ?? permission.enabled,
    })),
  );
    ...
    <div dir="rtl" className="space-y-4">
      <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-5">
        <h4 className="mb-4 flex items-center gap-2 text-sm font-semibold text-gray-900">
          ...
          {selectedAssistant ? `صلاحيات ${selectedAssistant.name}` : "اختر مساعداً لتعديل الصلاحيات"}
        </h4>
      ...
      <button
        ...
        disabled={!selectedAssistant || isSaving}
      ...
      >
        ...
        {isSaving ? "جارٍ حفظ الصلاحيات..." : "حفظ الصلاحيات"}
      </button>
    </div>
  );
}
